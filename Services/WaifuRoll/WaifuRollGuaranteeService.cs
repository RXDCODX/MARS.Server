...
) : IWaifuRollGuaranteeService
{
        ...
        if (!string.IsNullOrWhiteSpace(twitchId))
        {
            try
            {
                ...
                if (guarantee is { RollCount: >= GuaranteeRolls })
                {
                    ...
                    {
                        logger.LogWarning(
                            "Не удалось удалить пользователя {TwitchId} из системы гарантов",
                            twitchId
                        );
                    }

                    result = OperationResult<VipDropResponse>.Ok(
                        "VIP выпал по гаранту",
                        vipResponse
                    );
                }
                else
                {
                    ...
                    if (vipResponse.IsVipDropped)
                    {
                        ...
                        {
                            logger.LogWarning(
                                "Не удалось сбросить счетчик роллов для пользователя {TwitchId}",
                                twitchId
                            );
                        }
                    }
                    ...
                    result = OperationResult<VipDropResponse>.Ok(
                        "Проверка VIP статуса завершена",
                        vipResponse
                    );
                }
            }
            catch (Exception ex)
            {
                ...
                result = OperationResult<VipDropResponse>.Bad(
    ...
    {
        var result = OperationResult<RollCountResponse>.Bad(
            "Ошибка при увеличении счетчика роллов"
        );
        ...
        if (!string.IsNullOrWhiteSpace(twitchId))
        {
            try
            {
                ...
                result = OperationResult<RollCountResponse>.Ok(
                    "Счетчик роллов увеличен",
                    rollCountResponse
                );
            }
            catch (Exception ex)
            {
                ...
                result = OperationResult<RollCountResponse>.Bad(
    ...
    {
        var result = OperationResult<WaifuRollGuarantee?>.Bad(
            "Ошибка при получении информации о гаранте"
        );
        ...
        if (!string.IsNullOrWhiteSpace(twitchId))
        {
            try
            {
                ...
                result = OperationResult<WaifuRollGuarantee?>.Ok(
                    "Информация о гаранте получена",
                    guarantee
                );
            }
            catch (Exception ex)
            {
                ...
                result = OperationResult<WaifuRollGuarantee?>.Bad(
    ...
    {
        var result = OperationResult<bool>.Bad(
            "Ошибка при удалении пользователя из системы гарантов"
        );
            ...
            try
            {
                ...
                if (guarantee != null)
                {
                    ...
                    result = OperationResult<bool>.Ok(
                        "Пользователь удален из системы гарантов",
                        true
                    );
                }
                else
