...
public class CheckersGameManager
{
    ...
    private bool MakeRegularMove(char fromX, ushort fromY, char toX, ushort toY)
    {
        ...
        {
            // Неожиданная ситуация, возвращаем false или выбрасываем исключение, если нужно
            return false;
        }
    ...
    private bool MakeCaptureMove(char fromX, ushort fromY, char toX, ushort toY)
    {
        ...
        // Проверяем, что в исходной клетке действительно есть шашка
        if (fromCell?.Checker == null)
        {
            // Некорректное состояние: в исходной клетке нет шашки
            return false;
        }
        var checker = fromCell.Checker;
        fromCell.RemoveChecker();
    ...
    private void CheckGameEnd()
    {
        ...
        {
            Status = GameStatus.Finished;
            Winner = CurrentPlayer.Equals(Color.White) ? Color.Black : Color.White;
        }
    }
