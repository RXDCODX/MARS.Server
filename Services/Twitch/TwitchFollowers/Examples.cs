...
public static class Examples
{
    ...
    /// </summary>
    public static async Task<string> CheckUserStatusExample(
    ...
    )
    {
        try
        {
            ...
            return isModerator ? $"🛡️ {username} - модератор канала rxdcodx"
                : isVip ? $"⭐ {username} - VIP канала rxdcodx"
                : isFollower ? $"👥 {username} - фоловер канала rxdcodx"
                : $"👤 {username} - обычный зритель канала rxdcodx";
        }
        catch (Exception ex)
        {
            return $"❌ Ошибка: {ex.Message}";
    ...
    /// </summary>
    public static async Task<string> GetFollowersWithLimitExample(
        IRxdcodxViewersService viewersService,
        int limit = 10
    )
    {
    ...
    /// </summary>
    public static async Task<string> GetViPsWithLimitExample(
        IRxdcodxViewersService viewersService,
        int limit = 10
    )
    {
    ...
    /// </summary>
    public static async Task<string> ComprehensiveUserCheckExample(
        IRxdcodxViewersService viewersService,
        string userId,
        string username
    )
